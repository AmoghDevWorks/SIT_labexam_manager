...
const AdminDashboard = () => {
  ...
  const dashboardCards = [
    ...
    {
      id: 4,
      title: 'Subject Entry Control',
      description: 'Enable or disable subject data entry access for internal examiners',
      icon: (
        <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
        </svg>
      ),
      route: '/admin/manageSubjectEntry',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-100 via-blue-100 to-emerald-50">
      ...
      <header className="sticky top-0 z-50 bg-gradient-to-r from-[#0f1f3d] to-[#162847] shadow-[0_4px_24px_rgba(15,31,61,0.25)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 h-[60px] sm:h-[68px] md:h-[72px] flex items-center justify-between">
          <div>
            <h1 className="text-[10px] sm:text-xs md:text-sm font-extrabold text-white font-[Syne,sans-serif] tracking-wide leading-tight">
              Admin Dashboard
            </h1>
            <p className="text-[9px] sm:text-[10px] md:text-xs text-[#00c9a7]/70 font-[DM_Sans,sans-serif] font-medium tracking-wide leading-tight">
              Siddaganga Institute of Technology
            </p>
          </div>
          <div className="flex items-center gap-2 sm:gap-3">
            <span className="hidden sm:inline-flex text-[#00c9a7] text-[10px] md:text-xs font-bold tracking-widest uppercase px-3 md:px-4 py-1.5 rounded-full border border-[#00c9a7]/40 bg-[#00c9a7]/10 font-[Syne,sans-serif]">
              Admin Panel
            </span>
            <button
              onClick={handleLogout}
              className="group flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 bg-rose-500/10 hover:bg-rose-500 border border-rose-400/40 hover:border-rose-500 rounded-xl transition-all duration-200 hover:shadow-[0_4px_16px_rgba(239,68,68,0.3)]"
            >
              <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-rose-400 group-hover:text-white transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
              </svg>
              <span className="text-[10px] sm:text-xs font-bold text-rose-400 group-hover:text-white transition-colors font-[Syne,sans-serif]">
                Logout
              </span>
            </button>
          </div>
